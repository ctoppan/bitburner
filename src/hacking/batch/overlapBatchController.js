...
export async function main(ns) {
    ...
    while (true) {
        try {
            ...
            if (prepNeeded) {
                ns.print(`[overlap] Prep needed for ${target}. money=${formatMoney(ns, moneyNow)}/${formatMoney(ns, maxMoney)} sec=${secNow.toFixed(2)}/${minSec.toFixed(2)}`);
                await runPrepCycle(ns, hosts, target, workerRam, weakenScript, growScript);
                await ns.sleep(200);
                continue;
            }
            ...
            const growThreads = Math.max(1, Math.ceil(ns.growthAnalyze(target, 1 / Math.max(0.0001, 1 - actualHackPct))));
...
async function runPrepCycle(ns, hosts, target, workerRam, weakenScript, growScript) {
    const minSec = ns.getServerMinSecurityLevel(target);
