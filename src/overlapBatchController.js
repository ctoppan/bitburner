...
export async function main(ns) {
    ...
    let state = {
        ...
        maxBatches: clampInt(argMaxBatches, 128, 1200),
        maxJobs: 6000,
        tuneNote: "init",
        invest: "balanced",
        mode: "MULTI",
        ...
        try {
            ...
            if (canLaunchMore(state, activeJobs, activeBatches, fleet)) {
                ...
                if (launchTarget) {
                    ...
                    await tryLaunchBatch(ns, workers, launchTarget.target, state, targetInfo, fleet);
                }
            }
...
function tuneState(state, fleet, activeJobs, activeBatches, targetInfo) {
    ...
    if (ramFreeRatio > 0.85) {
        ...
        state.spacer = clampInt(Math.floor(state.spacer * 0.92), 20, 400);
        state.maxBatches = clampInt(state.maxBatches + 48, 128, 1200);
        state.maxJobs = clampInt(state.maxJobs + 400, 1200, 12000);
    ...
    if (ramFreeRatio > 0.60) {
        ...
        state.spacer = clampInt(Math.floor(state.spacer * 0.95), 25, 400);
        state.maxBatches = clampInt(state.maxBatches + 24, 128, 1200);
        state.maxJobs = clampInt(state.maxJobs + 250, 1200, 12000);
        state.invest = "buy_servers";
        state.mode = "MULTI";
    ...
    if (ramFreeRatio < 0.10) {
        state.hackPct = clamp(state.hackPct * 0.88, 0.01, 0.16);
        state.spacer = clampInt(Math.floor(state.spacer * 1.15), 30, 500);
        state.maxBatches = clampInt(state.maxBatches - 32, 64, 1200);
    ...
    if (jobPressure > 0.95 && ramFreeRatio > 0.20) {
        state.maxJobs = clampInt(state.maxJobs + 500, 1200, 12000);
        state.maxBatches = clampInt(state.maxBatches + 32, 128, 1200);
        state.invest = "buy_servers";
...
async function tryLaunchBatch(ns, workers, target, state, targetInfo, fleet) {
    ...
    const hackFraction = clamp(state.hackPct, 0.04, 0.20);

    let hackThreads = Math.max(6, Math.floor(ns.hackAnalyzeThreads(target, moneyMax * hackFraction) || 6));
    const growMultiplier = 1 / Math.max(0.01, 1 - hackFraction);
    let growThreads = Math.max(24, Math.ceil(ns.growthAnalyze(target, growMultiplier) || 24));
    ...
    const ramScale = clamp(Math.floor(fleet.free / 4096), 1, 64);
    if (fleet.free > 8192) {
        const scale = Math.min(ramScale, 16);
        hackThreads = Math.max(6, Math.floor(hackThreads * scale));
        growThreads = Math.max(24, Math.floor(growThreads * scale));
...
function chooseActiveTargets(rankedTargets, fleet) {
    ...
    if (freeRatio > 0.80) targetCount = 6;
    else if (freeRatio > 0.60) targetCount = 5;
    else if (freeRatio > 0.40) targetCount = 4;
    else if (freeRatio > 0.20) targetCount = 3;

    return rankedTargets.slice(0, targetCount);
}
...
function buildSummary(ns, state, fleet, target, targetScore, targetInfo, activeJobs, activeBatches, activeTargets, rankedTargets) {
    const lines = [
