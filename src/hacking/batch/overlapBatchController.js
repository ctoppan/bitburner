  ...
  while (true) {
    try {
      ...
      if (batchable.length > 0 && fleetFreeRam >= CFG.minFleetFreeForBatchGb) {
        ...
        const chosen = chooseBatchTarget(ns, batchable, prepWaveCounts, CFG);
        ...
        runBatchWave(ns, hosts, chosen.host, CFG.defaultHackPct, spacingMs, hackScript, growScript, weakenScript);
...
function chooseBatchTarget(ns, batchable, prepWaveCounts, cfg) {
...
function runBatchWave(ns, hosts, target, desiredHackPct, spacingMs, hackScript, growScript, weakenScript) {
